    ...
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}[1]
        ...
        \Function{extend-0}{prefix, p::suffix, $s$}
          \For{$\langle s, s', p\rangle\footnote{An edge is represented using
          $\langle s, s', p\rangle$ tuple when there is no ambiguity.} \in E$}
            \If{suffix = \nil}
              {\Return{prefix@p}}
...
The main routines of the algorithm is shown in Algorithm~\ref{alg:search}.  In
